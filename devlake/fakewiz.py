...
The seam is ``ingest._post`` (``brick/ingest.py``): ``fetch_findings`` calls it once per page
and reads the connection's ``nodes`` / ``pageInfo`` straight out of the returned payload, with
no error handling of its own -- a caller that got back an error-shaped body would read
``connection.get("nodes") or []`` and silently yield nothing. That is precisely the failure this
package exists to make loud: ``FakeWiz`` reproduces the one thing a real tenant would refuse on
-- a filter value in the wrong shape for its scope's filter type -- by raising the same
``RuntimeError`` ``_post`` itself raises on an HTTP 4xx, formatted through ``ingest``'s own
``describe_errors`` so the message a test sees is the message a real 400 would have produced.
...
``source.connection`` lookup, real ``_severity_gate``. Everything below the seam is this module.
"""
...
_UNKNOWN_SCOPE = "FakeWiz: unknown scope {scope!r} -- expected one of {expected}"

#: A defensive fallback only -- ``brick/config.py`` defines ``OBJECT_FILTERS`` for every scope
#: today (``os``, ``sca``, ``sast``), so this branch does not currently fire. It stays as the
#: table a scope *would* fall back to if it had no ``OBJECT_FILTERS`` entry of its own --
#: ``projectIdV2`` is the one wrapped key ``VulnerabilityFindingFilters`` has.
_OBJECT_FILTERS_FALLBACK = {
    "os": ("projectIdV2",),
    "all": ("projectIdV2",),
}
...
class FakeWiz:
    """A fake GraphQL server for one ``scope``, backed by an in-memory node list.
    ...
    def __init__(
        ...
        *,
        ...
        self.scope = scope
        self.ingest_module = ingest_module
        self.calls: List[Dict[str, Any]] = []

        scopes = getattr(ingest_module, "SCOPES", None)
        if scopes is not None and scope not in scopes:
            raise RuntimeError(_UNKNOWN_SCOPE.format(scope=scope, expected=sorted(scopes)))
        self.connection = self._resolve_connection(scope, ingest_module)
    ...
    @staticmethod
    def _resolve_connection(scope: str, ingest_module: Any) -> str:
        """The GraphQL field this scope answers under.

        ``ingest`` re-exports ``config.SOURCES`` by name (``from config import ... SOURCES``),
        so ``SOURCES[scope].connection`` names it directly -- ``os`` and ``sca`` read
        ``vulnerabilityFindings``, ``sast`` reads ``sastFindings``. The bare
        ``"vulnerabilityFindings"`` fallback below is defensive only: every scope
        ``brick/config.py`` defines has a ``SOURCES`` entry today.
        """
        sources = getattr(ingest_module, "SOURCES", None)
        if sources is None:
            return "vulnerabilityFindings"
        source = sources.get(scope)
        if source is None:
            raise RuntimeError(
                _UNKNOWN_SCOPE.format(scope=scope, expected=sorted(sources))
            )
    ...
    def _object_filters(self) -> Dict[str, Sequence[str]]:
        ...
        ``getattr`` rather than ``import config``: ``ingest`` imports ``OBJECT_FILTERS`` by name
        (``from config import ... OBJECT_FILTERS``), so it is already an attribute of the
        *loaded* ``ingest`` module -- the one ``devlake.session.put_brick_on_path`` put on
        ``sys.path``. The fallback (see its own comment) does not currently fire: every scope
        has an ``OBJECT_FILTERS`` entry.
        """
        return getattr(self.ingest_module, "OBJECT_FILTERS", None) or _OBJECT_FILTERS_FALLBACK
    ...
    def _refuse(self, api_url: str, key: str, value: Any, *, expected: str) -> None:
        ...
        describe_errors = self.ingest_module.describe_errors
    ...
    def post(
        ...
        """The ``ingest._post`` replacement. Same positional shape ``ingest.py`` calls it with,
        plus the optional ``query=`` some call sites pass through."""
        self.calls.append(
...
def install(
    ...
    **``run_pipeline_module`` is not optional in practice, and this is the surprise Step 3
    turned up.** ``run_pipeline.py`` imports ``get_token`` with ``from ingest import ...
