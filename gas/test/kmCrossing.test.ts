...
// together in remediation.ts).
//
// PERTURBATION, INLINE: the bare comparison this file guards against is reproduced below
// rather than merely described, so a later "simplification" that reintroduces it fails this
// test rather than silently regressing the published quantile by one event.
...
describe("the crossing epsilon, on CLAUDE.md's own example", () => {
  ...
  it("PERTURBATION: a bare `s <= threshold` reports 10, because the ULP leans the wrong way",
    () => {
      const times = TEN.map((r) => r.mttr_days);
