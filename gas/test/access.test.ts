...
// to a vulnerability register — silently, because an admitted user looks exactly like an
// intended one. Failing closed locks the owner out of the only place the allowlist can be
// edited. So the specs below pin BOTH: every path that must deny, and the one identity that
// must never be deniable.
//
...
describe("the guards hand back the shapes their call sites expect", () => {
  ...
  it("deniedPage returns null for an allowed caller", async () => {
    // The null is what lets entry.js write `deniedPage() || Server.doGet(e)` — a truthy
    // return for an allowed user would swallow the app.
