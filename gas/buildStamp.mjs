...
//                  `npm run which-build` resolves.
//
// WHY THERE IS NO COMMIT STAMP HERE
//
// The obvious alternative is a git SHA baked in at build time, and it does not work: dist/
// is tracked in this repo, so the order is always build, then commit src and dist
...
// new commit, which makes it stale again. The symptom would be that every `npm run check`
// after any commit leaves dist/ dirty by exactly one line, forever, with no sequence of
...
// and only a real source change flips it.
//
// The lookup a SHA would have existed for is now `npm run which-build`, which replays this
// same hash across history. That answers strictly more than an embedded SHA could: it names
