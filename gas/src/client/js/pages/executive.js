...
export async function renderExecutive(main, _params, ctx) {
  ...
  // AFTER THE SEVERITY STRIP AND BEFORE THE BY-DOMAIN TABLE, which is "after the fix-next
  // block" in the sense that matters: the page reads hero (how fast risk closes) -> fix next
  // (what to do) -> the open distribution (what there is) -> the cold zone (where none of it is
  // moving) -> who owns it -> when we last looked. Putting it between Fix next and the severity
  // strip would split the two blocks a leader reads as a pair — the ranked list and the
  // distribution it was ranked out of — and the cold zone is the first block on this page that
  // is about ABSENCE of movement rather than about the backlog itself, which is the turn the
  // by-domain table then answers ("whose").
  ...
  main.append(noticeHost, heroHost, fixHost, sevHost, coldHost, byDomainHost, scanHost);
