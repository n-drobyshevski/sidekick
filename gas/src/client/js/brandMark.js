...
// a 22 px header glyph render at a third of a pixel each and read as grey noise, while
// the shield and the orbit are chunky black forms that survive it. So the app header takes
// the compact crop and the boot splash, which has a whole screen, takes the full mark.
//
// COLOUR. The three colours are the artwork's own and are deliberately NOT severity
// tokens. They ride `--mark-*` custom properties applied by class from styles.css, with
// the literal hex kept on the presentation attribute underneath — the same arrangement as
// uiIcons.js, so the mark still draws correctly with no stylesheet at all. That matters
// here more than anywhere else in the app: the first copy of this mark paints from
// index.html before any JavaScript has run.
//
// SHARED WITH gas_ai/. This is the Wiz Sidekick mark, not one product's — the file is a
// verbatim copy of gas_ai/src/client/js/ui/brandMark.js apart from the import above, and
// should stay one. A change to the artwork belongs in both.

import { svgEl } from "./uiIcons.js";
 ...
 *
 * Decorative by default — this thing sits next to the words "Wiz Sidekick OS" in both
 * places it appears, and announcing the picture as well as the name would say it twice.
 * `label` is for a caller that hides the wordmark and leaves the mark as the only identity
 * on screen; nothing in this app does that today.
 */
