...
/**
 * THE BASE FILTER, IN WORDS — one short phrase per key the query carries that is not the
 * severity gate and not the status list.
 *
 * The Overview line has to name what the measurement kept OUT, and a filter that removes rows
 * is exactly that: every one of these makes a count fall, and none of them can be reported as
 * a zero (CLAUDE.md, "The Outside is everything the measurement kept out"). Severity is the
 * scan's own gate and is stated separately (it is not even in BASE_VARIABLES — buildVariables
 * injects it); status is the register's definition of a finding rather than a narrowing of the
 * population, and naming it would read as an exclusion that is not one.
 *
 * WHY THIS LIVES HERE AND NOT IN `wizQuery.ts`: that file is GENERATED by
 * `test/extract_query.py` from `os_vulns.py`'s QUERY/VARIABLES, and its own header says
 * "regenerate rather than hand-editing" — hand-written prose there is prose the next
 * regeneration deletes. This module is the hand-written half of the same filter (it is where
 * `projectIdV2` and `severity` are injected), so the words sit beside the assembly instead.
 *
 * `test/wizQueryWords.test.ts` binds the count to `BASE_VARIABLES.filterBy`'s own keys, so a
 * filter added upstream cannot reach the screen unnamed. Read what the filter SAYS — the
 * `hasFix: true` that os_vulns.py still carries is NOT in this register's baseline (it was
 * dropped deliberately; see REMEDIATION_ROLLOUT_ISO), so there is no word for it here.
 */
export const BASE_FILTER_WORDS: string[] = [
  "one Wiz project", // projectIdV2
  "virtual machines only", // assetType
  "OS-level detections", // detectionMethod
  "openssl, python and vim excluded", // detailedNameV2.notEquals
  "no representative-resource stand-ins", // assetIsRepresentativeResource
];

/** Filter keys the words above deliberately do not name: the gate, and what a finding IS. */
export const UNNAMED_FILTER_KEYS = ["severity", "status"] as const;
