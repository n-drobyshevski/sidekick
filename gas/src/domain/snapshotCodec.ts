...
// it — measured in production at 4.06 MB gzipped and 2–4.5 s to ungzip and parse, the largest
// single cost left on a cold page. Almost all of that text is repetition: every one of ~58k rows
...
// rows), which for the ledger's uniform rows is each row's own order. test/snapshotCodec.test.ts
// holds that on random and adversarial rows.
...
export interface SnapshotV2 {
  ...
  /** Present only when some ledger key differs from its row's `vuln_key`. */
  ledgerKeys?: string[];
  episodeTable: EncodedTable;
}

/** The ledger map and episode list as a v2 snapshot. One string table serves both. */
export function encodeSnapshot(ledger: Record<string, Row>, episodes: Row[]): SnapshotV2 {
