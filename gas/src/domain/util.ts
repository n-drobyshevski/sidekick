...
export function parseTs(v: unknown): number | null {
  ...
  // no-timezone suffix below, so the two regex tests are pure overhead — and this runs millions
  // of times per request. A NaN falls through to the general path, which answers the same.
