...
A Google Apps Script web app for Wiz OS vulnerability operations: OS-level CVEs on host workloads, exploitability triage, and MTTR / SLA
...
The Python `wiz_dashboard/domain/` layer is the behavioral spec for the _ported_
modules: the TypeScript `src/domain/` ports are verified against golden fixtures
generated by running the _original_ Python code (see `test/export_fixtures.py`,
...
GAS-first and are covered by hand-written vitest specs instead of fixture parity.
...
3. **A one-shot backfill recovers already-compacted history.** The Drive checkpoint chain is
   ...
   skips episode keys, so the data was unreachable rather than gone. Settings → _Domain-tag
   backfill_ reads the latest checkpoint and reports `recovered` / `alreadyHad` /
...
4. **The re-listed row is read before it is dropped.** The vulnerability query fetches
   `status: [OPEN, RESOLVED]`, so Wiz hands back these sealed lifecycles on _every_ scan, each
...
5. **The history backfill reaches what the checkpoint cannot.** Legacy imported episodes were
   ...
   records they came from. Settings → _History backfill_ merges tag bags in the same walk that
...
- **Show findings without a vendor fix** (Settings, default on). Findings awaiting a
  ...
  from _every_ finding-derived surface (charts, tables, KPIs, exports, Attribution, Scan
...
- **Latency — the wait for a fix to _exist_.** Every clock above measures the wait to
  _deploy_ a fix. `latencyView` (`src/domain/remediation.ts`) measures the wait for one to
  ...
  - `"detection"` — `first_seen → fix_available_at`. How long _we_ waited after finding it.
  ...
  fixed and measure how fast the fixed ones were fixed. Note what is _not_ done — those rows
  ...
  **The show-no-fix toggle does not apply to it.** Those rows are exactly the censored
  population, so `api.ts` computes latency from the base _before_ that filter and the page
...
`has_exploit` and `epss` are `LedgerRow` columns (see _Sticky signals_), so tiers survive
...
- **One SLA number per fleet.** The page runs `openPastSla` over `actionableView`, the
  ...
  findings whose clock has _started_, which is smaller than the open count whenever anything
...
against `#ea580c` separate by ΔE **6.7** for _normal_ vision and **1.6** under deuteranopia,
...
_Migrating from the Apps Script app_.
...
the unit that matters (_"room for about N more tracked vulnerabilities"_, derived from the live
...
Compaction never _removes_ anything — it moves settled lifecycles from `vuln_ledger` into
...
- **Scan rows get their `severities` scope narrowed too.** A quick refresh builds its delta
  query from the _baseline scan row's_ scope, "never the current settings" — so dropping a
...
`deleteScansCore` seeds the rebuilt ledger from the checkpoint _minus the keys present in
`resolved_episodes`_, so removing an episode row un-masks its checkpoint entry and the next
scan deletion restores it as a live RESOLVED `vuln_ledger` row.

Unlike compaction — which is _gated_ on leaving MTTR, trend and coverage identical — this
...
5. Set the remaining Script Properties by hand (never committed anywhere):
   ...
   - `WIZ_SUPPORT_GROUP_TAG_KEY` _(optional)_ — the subscription tag whose value is the
     Support Group. Defaults to `Wiz/provisioning`; set it only if your tenant uses a
     different tag key.
   - `WIZ_DOMAIN_TAG_KEY` _(optional)_ — the **resource** tag whose value is the **domain**
   ...
   - `ALLOWED_USERS` _(optional but read fail-closed)_ — comma/semicolon/whitespace-separated
   ...
   - `ALLOWED_ADMINS` _(optional)_ — addresses allowed to edit `ALLOWED_USERS` from
...
(`src/server/welcome.ts`) serves a card first: _"This dashboard will open as
alex@example.com"_, a **Continue** button, and a **Switch Google account** link.
...
credential to collect. What the screen is _for_ is the Apps Script multiple-accounts trap: a
...
it and the screen returns in the morning. Six _idle_ hours mid-day also brings it back. That is
...
Past the fetch phase a _healthy_ run can't be interrupted — the `scans` row is seconds away
...
- **Stop escalates by phase.** Once a job has been silent for `STALE_JOB_MS` (30 min) the card
  ...
  `cancelScan` runs `recoverIfNeeded()` first, so a killed mid-write is _rolled back from its
  journal_, never cancelled outright — cancelling would trash an archive a committed `scans`
...
- [ ] **Severity purge, end to end.** Note the Storage cell count, purge LOW+MEDIUM, and
      confirm the number _drops_ (a no-op means the ledger had no allocated slack, not that
