"""Pure unit tests for the domain rule engine.
...
def test_priority_first_match_wins():
    items = [_domain("First", [tag("env", "prod")]), _domain("Second", [tag("env", "prod")])]
...
def test_empty_rules_and_malformed_conditions_never_match():
    ...
    assert assign([{"name": "Empty", "rules": [{"conditions": []}]}], _node()) == dr.UNASSIGNED
    # a malformed condition poisons its whole rule (fail closed, not fail open)
    items = [
        {"name": "Poisoned", "rules": [{"conditions": [tag("env", "prod"), {"type": "nope"}]}]}
    ]
...
def test_validate_ok_for_well_formed_items():
    items = [
        _domain("Pay", [tag("team", "payments")], [sub("core-prod")]),
        _domain("Web", [rx("^web-")]),
    ]
...
NODES = [
    ...
    _node(name="batch-fn-01", fid="f5"),
    {
        ...
        "name": "CVE-2024-9999",
        "severity": "LOW",
        "vulnerableAsset": {"name": "cache-01", "type": "VM", "cloudPlatform": "GCP", "tags": None},
    },
]
EXPECTED = ["Payments", "Web", "Payments", "Everything-Staging", dr.UNASSIGNED, dr.UNASSIGNED]
...
def test_assign_domains_frame_handles_flattened_tag_columns_only():
    ...
    nodes = [
        _node(name="a", tags={"team": "payments"}, fid="x1"),
...
def test_assign_domains_ledger_all_condition_types():
    ...
    assert list(got) == ["Web", "Payments", "Everything-Staging", dr.UNASSIGNED, dr.UNASSIGNED]
...
def test_compacted_episode_with_a_tag_bag_is_claimable_by_a_tag_rule():
    ...
    assert [dr.assign_domain(r, compiled) for r in df.to_dict("records")] == [
        "Payments",
        dr.UNASSIGNED,
        dr.UNASSIGNED,
    ]
...
def test_ledger_null_inputs_only_name_regex_can_classify():
    items = [_domain("ByName", [rx("^legacy-")]), _domain("ByTag", [tag("env", "prod")])]
