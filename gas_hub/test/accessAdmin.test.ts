...
//
// PORTED FROM gas_devsecops/test/accessAdmin.test.ts, which is itself gas's. What is
// DIFFERENT here is the three `vi.mock` blocks that are gone: the registers had to stub
// `sheetsDb`, `readModelStore` and `locks` because importing their `api.ts` drags Sheets and
// Drive in with the ledger, and only the authorization path was under test. This app's api.ts
// imports `buildInfo`, `access`, `props` and `urls` and nothing else — there is no ledger, no
// spreadsheet and no lock — so the module loads under the two stubs below alone. Deleting
// those mocks rather than keeping them dormant is deliberate: a `vi.mock` of a module that
// does not exist is a rewiring nobody can see the effect of, and this file is already in the
// isolated vitest project for the two it does keep.
//
...
// =========================================================================================
//  THE SAME BOUNDARY, ONE ENDPOINT FURTHER — the three sibling URLs
// =========================================================================================
//
// NEW HERE, because this app has an admin surface the registers do not: `api_saveUrls` writes
// what becomes the `href` of every tile on the front page, for everybody. It is reachable
// from any allowed caller's browser console exactly as `api_saveAccess` is, so it re-checks
// `canEditUsers()` server-side and does not trust the panel to have hidden itself. `getUrls`
// follows `getAccess`'s shape for the same reason: a non-editor gets the flag and not the
// values — not because the addresses are secret (every allowed caller reads them off the
// tiles) but because a payload shaped "here is the editable state" invites a form that will
// be refused on save.
describe("the sidekick URLs are behind the same tier", () => {
  it("lets an admin set them — the same delegation as the people list", async () => {
    activeEmail = "admin@example.com";
    const { saveUrls } = await load();
    const res = saveUrls({ urls: { os: "http://localhost:8787/" } });
    expect(res.ok).toBe(true);
    expect(props.URL_OS).toBe("http://localhost:8787/");
  });

  it("REFUSES a merely-listed user, whatever the panel drew", async () => {
    activeEmail = "listed@example.com";
    const { saveUrls } = await load();
    const res = saveUrls({ urls: { os: "http://localhost:8787/" } });
    expect(res.ok).toBe(false);
    expect(props.URL_OS, "a non-editor's URL reached the property").toBeUndefined();
  });

  it("refuses a value that is not a sidekick URL, and stores nothing", async () => {
    const { saveUrls } = await load();
    const res = saveUrls({ urls: { os: "https://evil.example/exec" } });
    expect(res.ok).toBe(false);
    expect(props.URL_OS).toBeUndefined();
  });

  it("hands a non-editor the flag and NOT the addresses", async () => {
    activeEmail = "listed@example.com";
    const { getUrls } = await load();
    const data = getUrls().data as Record<string, unknown>;
    expect(data["canEditUrls"]).toBe(false);
    expect(data["urls"]).toBeUndefined();
  });

  it("hands an editor both", async () => {
    props.URL_AI = "http://localhost:8788/";
    const { getUrls } = await load();
    const data = getUrls().data as Record<string, unknown>;
    expect(data["canEditUrls"]).toBe(true);
    expect(data["urls"]).toEqual({ os: "", ai: "http://localhost:8788/", devsecops: "" });
  });

  it("answers the AFTER-URLS OBJECT ITSELF, not a { urls } wrapper", async () => {
    // The two endpoints are NOT symmetric: `getUrls` wraps its payload, `saveUrls` returns
    // `after` directly. pages/settings.js reads the real shape (`Object.assign(saved,
    // result)`) and its own comment says so; this is what would fail if either side were
    // "tidied" to match the other and the panel silently stopped reflecting a save.
    const { saveUrls } = await load();
    const res = saveUrls({ urls: { devsecops: "http://localhost:8789/" } });
    expect(res.data).toEqual({ os: "", ai: "", devsecops: "http://localhost:8789/" });
  });
});
