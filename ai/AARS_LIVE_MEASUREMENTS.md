...
| # | Defect | Evidence |
|---|---|---|
| A | **`LINEAGE` yields 590 rows and 0 edges.** `edge_count` is 167 before and after the pagination fix. `assetEdges` filters only `HAS_ISSUE`, so nothing is lost at persistence — the drop is inside `normalizeLineagePage`. Its first guard, `if (entities.length !== slots.length) continue`, discards whole rows silently and is the obvious suspect; but slot count derives from the relationship legs, not the roots, so root-narrowing should not move it. **Not confirmed.** | `stepRows.LINEAGE = 590`, `edge_count` 167 unchanged |
