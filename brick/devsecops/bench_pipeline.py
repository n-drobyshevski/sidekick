...
Ported from ``brick/bench_pipeline.py`` when this fork absorbed the ``os`` scope (S2); nothing
about the timing instrument is scope-specific, so it measures whichever scope the synthetic
register below is built for (``os`` by default -- see ``run()``).
...
lands with a before/after number from here, or it does not land.
    ...
    python brick/devsecops/bench_pipeline.py --findings 137870 --scans 3 --out before.json \\
        --dump before/

    # after the change
    python brick/devsecops/bench_pipeline.py --findings 137870 --scans 3 --out after.json \\
