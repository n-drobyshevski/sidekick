...
Ported from ``brick/tests/test_bundle.py`` when this fork absorbed the ``os`` scope (S2): every
job in ``brick/databricks.yml`` now points at ``brick/devsecops/run_pipeline.py``, so ``FORK_OF``
-- which used to map two entry points to two forks' ``config.py`` -- collapses to the one entry.
The scope check this module exists for is no weaker for it: a job could still be pointed at
``brick/run_pipeline.py`` by mistake (that file still exists, pending its own retirement), and
``test_every_scope_belongs_to_the_fork_the_job_points_at`` would refuse it the same way it always
has, because ``FORK_OF`` only recognises the one path the bundle is supposed to use.

``config.SCOPES`` is read with ``ast`` rather than imported: the two forks still carry the same
module names and exactly one of them may be on ``sys.path`` (see
...
# The bundle lives at brick/databricks.yml, not brick/devsecops/databricks.yml -- it deploys
# every scope's job now, and that has not moved with this file.
BUNDLE = BRICK_DIR / "databricks.yml"

#: Which fork each entry point belongs to, and therefore whose ``SCOPES`` its ``--scope`` is
#: checked against. One entry now: every job in the bundle points at this fork's
#: ``run_pipeline.py``, including the ``os`` job it absorbed from ``brick/`` (S2).
FORK_OF = {
    "brick/devsecops/run_pipeline.py": DEVSECOPS_DIR / "config.py",
}
