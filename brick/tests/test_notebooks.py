...
def test_no_cell_reads_an_append_only_table_directly(notebook):
    ...
    ``metrics`` -- which is also the ``metrics.py`` module, imported by that name in 06's and
    07's "module versions" diagnostic cell (``import config, dbx, ingest, ledger, metrics,
