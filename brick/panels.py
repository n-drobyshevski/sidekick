...
told apart by ``family`` (``mttr``, ``program``, ``capacity``, ``scan``). Nothing is overwritten,
so every read has to name a scan or it silently blends every run that has ever happened into one
plausible-looking chart -- and it has to name a family, or it blends grains that share no key.
Rather than repeat ``scan_id = (SELECT max_by(scan_id, scan_ts) ...)`` in every cell and hope,
``context()`` registers session temp views that are *already* pinned, scoped, family-filtered and
severity-filtered:
...
# One definition of the seven subsets, shared with `metrics.rule_sensitivity`. The sweep is not
# published as a family -- it is recomputed here from the lifecycles, which is why `metrics`
# carries no `sensitivity` grain -- and the two still walk one list, because two would drift and
# the failure would be a page that disagrees with the transform about what "KEV or EPSS" means.
