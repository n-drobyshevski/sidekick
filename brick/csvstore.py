...
#: Every table, in the order a reader wants them. ``Tables`` attribute names, so this and the
#: dataclass cannot drift -- a table added to one and not the other fails at import, not on the
#: day somebody tries to restore.
TABLE_ATTRS = (
    "scans",
    "ledger",
    "mttr",
