...
# --------------------------------------------------------------------------- #
#  SLA posture — bullet small-multiples (median MTTR vs SLA target)
# --------------------------------------------------------------------------- #
# Measure-bar colours: green when the median is within target, red when it overruns.
# Severity-neutral (the severity is the lane label); mirrors the .sla-bullet__fill--*
# tokens in assets/styles.css. The ✅/⚠️/🚨 status column in render_mttr_widget keeps
# the policy thresholds non-color too, and that table is this chart's a11y fallback.
_SLA_MET = "#16a34a"
_SLA_MISSED = "#dc2626"
_SLA_TARGET_MARK = "#0a0a0a"


def build_sla_bullets(per_sev: dict):
    """Bullet small-multiples: per-severity median MTTR (bar) vs SLA target.

    One lane per severity that has both a computable median and an SLA target. Each lane
    STATES its own target in the row label (e.g. ``Critical · ≤7d``) and labels the bar with
    the median in the same friendly units as the table (``format_duration``), so the lanes
    read on their own now that the full table is demoted to an expander. The bar is green
    within target / red over it, with a dark tick at the target. Per-lane x-axes are dropped
    (four independent numeric scales were noise, and a shared scale can't fit 7d and 180d in
    one row); the stated target + tick + colour carry the verdict, not bar length. Returns
    ``None`` when no severity has both a median and a target (e.g. nothing resolved)."""
    rows = []
    for sev in SEVERITY_ORDER:
        if sev == "INFO":
            continue  # Info is excluded from the SLA posture (matches the severity breakdown's 4 levels)
        d = per_sev.get(sev)
        if not d:
            continue
        median = d.get("mttr_median")
        target = d.get("sla_target")
        if median is None or pd.isna(median) or not target:
            continue
        median, target = float(median), float(target)
        met = median <= target
        rows.append(
            {
                "severity": sev,
                "lane": f"{sev.title()} · ≤{int(target)}d",
                "median": round(median, 2),
                "target": target,
                "met": bool(met),
                "median_label": format_duration(median),
                "verdict": "Within SLA" if met else f"{median / target:.1f}× over target",
                # Pin each lane's independent x-domain wide enough that the bar-end label has
                # room — fixes the old clipped "20"/"30" labels that sat at the axis max.
                "axis_max": max(median, target) * 1.28,
                "sla_pct": (None if d.get("sla_pct") is None else round(float(d["sla_pct"]), 1)),
    ...
    order = [r["lane"] for r in rows]
    tooltip = [
        alt.Tooltip("lane:N", title="Severity"),
        alt.Tooltip("median_label:N", title="Median"),
        alt.Tooltip("target:Q", title="SLA target (days)"),
        alt.Tooltip("verdict:N", title="Verdict"),
        alt.Tooltip("sla_pct:Q", title="In SLA %", format=".0f"),
        alt.Tooltip("resolved:Q", title="Resolved"),
        alt.Tooltip("open:Q", title="Open"),
    ]
    bar = alt.Chart().mark_bar(height=14, cornerRadiusEnd=3).encode(
        x=alt.X("median:Q", title=None, axis=None),
        color=alt.Color(
            "met:N",
            scale=alt.Scale(domain=[True, False], range=[_SLA_MET, _SLA_MISSED]),
            legend=None,
        ),
        tooltip=tooltip,
    )
    # Friendly median at the bar end (matches the table's Median column; figures in tooltip).
    label = alt.Chart().mark_text(align="left", baseline="middle", dx=6, fontSize=11).encode(
        x=alt.X("median:Q", axis=None),
        text=alt.Text("median_label:N"),
        tooltip=tooltip,
    )
    # SLA target marker — a dark tick the bar stays left of when in SLA.
    target = alt.Chart().mark_tick(thickness=2.5, size=22, color=_SLA_TARGET_MARK).encode(
        x=alt.X("target:Q", title=None, axis=None),
        tooltip=tooltip,
    )
    # Invisible mark that widens each lane's independent x-domain so the label has headroom.
    extent = alt.Chart().mark_tick(opacity=0).encode(x=alt.X("axis_max:Q", axis=None))
    return (
        alt.layer(bar, label, target, extent, data=data)
...
def sla_bullets(per_sev: dict, *, empty="No remediation data yet — SLA posture builds up as findings are resolved.") -> None:
    """Render the SLA bullet small-multiples, or a caption when nothing is resolved."""
    chart = build_sla_bullets(per_sev)
    if chart is None:
        st.caption(empty)
        return
    st.altair_chart(chart, width="stretch")
