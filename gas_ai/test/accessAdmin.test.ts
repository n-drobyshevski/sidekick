...
// The register side of api.ts is irrelevant here and would drag Sheets in on import. Only the
// symbols api.ts binds at module scope need to exist.
...
describe("the tier stops where it has to", () => {
  it("REFUSES an admin editing the admins list", async () => {
  ...
  it("does not let an admin promote themselves by editing the PEOPLE list", async () => {
    // The other way round the tier could collapse: admin standing lives in ALLOWED_ADMINS
    // alone, so writing an address into ALLOWED_USERS grants nothing beyond opening the app.
    activeEmail = "admin@example.com";
    const { saveAccess, getAccess } = await load();
    expect(saveAccess({ users: "listed@example.com, admin@example.com" }).ok).toBe(true);
    expect(props["ALLOWED_ADMINS"]).toBe("admin@example.com");
    expect((getAccess().data as Record<string, unknown>)["canEditAdmins"]).toBe(false);
  });
});
