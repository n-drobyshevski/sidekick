...
// walker, pointed at gas_ai via --root — see that file's own header on why `--root` is what
// lets one script walk all four apps). The walker and its DOM-free half (`densityModel.mjs`)
// stay in gas_devsecops, per this package's own scope: gas_ai does not get a copy of either,
// only this one test that pins the one thing specific to THIS app's PAGES table.
//
...
// already pins those against hand-built fixtures, and `gas/test/density.test.js` is the
// template this file ports. This file's only job is the one thing a shared fixture cannot
// stand in for: that `parsePages()` reads gas_ai's REAL pages.js and comes back with gas_ai's
...
describe("parsePages() against gas_ai's own pages.js — the route list `npm run density` walks", () => {
  it("yields exactly gas_ai's eleven routes, in PAGES' own (= rail) order", () => {
    ...
    // against itself. In rail order: the Landscape lane (graph, inventory), the Risk lane
    // (problems — this register's front door — combos, config), the Assurance lane
  ...
  it("also names each route's own render function, so a route that pointed at the wrong "
    ...
    const byRoute = Object.fromEntries(
      parsePages(PAGES_JS).map((p) => [p.route, p.render]),
    );
    expect(byRoute).toEqual({
      ...
      combos: "renderCombos",
      config: "renderConfigFindings",
      compliance: "renderCompliance",
      scans: "renderScans",
      aars: "renderAarsRules",
      data: "renderData",
  ...
  it("PERTURBATION: an empty PAGES literal yields zero routes, and the zero-routes guard "
    ...
    const routes = parsePages("export const PAGES = {};").map((p) => p.route);
    expect(routes).toEqual([]);
    function refuseIfEmpty(allRoutes) {
      if (!allRoutes.length) {
        throw new Error(
          "parsePages() found no routes in pages.js's PAGES table — refusing to report an "
    ...
    expect(() => refuseIfEmpty(parsePages(PAGES_JS).map((p) => p.route))).not.toThrow();
  });
});
