// PORTED FROM `gas/test/dayAxisTicks.test.js` ALONGSIDE THE CODE IT GUARDS. This app had no
// day axis at all before that port, so it has never had this defect on screen — the tests come
// with the mechanism rather than after it, which is the only way a thinning rule that fires on
// a shape nobody has hit yet is anything but decoration. The fixtures are `gas/`'s measurements
// off `gas/`'s harness and stay that way: the defect is a property of `fmtDay`'s eleven
// characters against Chart.js's five-character tick budget, which all three registers share.
//
...
// The two functions under test are pure, so stubbing the one global the module evaluation
// touches and awaiting the import at top level is enough.
