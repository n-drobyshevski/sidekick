...
// carries serverCache.CACHE_EPOCH instead (gas/ #322, gas_devsecops #329), and a change to one
// payload's shape or meaning is expressed by bumping that payload's namespace — `problemsModel1` →
// `problemsModel2`. A namespace with no number has nothing to bump, so it cannot be introduced.
//
...
describe("cache namespaces", () => {
  it("finds the read-models this spec expects", () => {
    ...
    for (const n of ["bootstrapCore1", "problemsModel1", "assetsModel2", "actionsRanked1", "settingsImpact1"]) {
