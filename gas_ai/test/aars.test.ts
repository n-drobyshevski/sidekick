...
describe("computeAars — applied table rows", () => {
  it("Agent-A / Agent-B / agent-F / agent-F-preprod / Agent-E → 62 HIGH", () => {
    ...
    expect(r.pillars).toEqual({ toxic: 20, compliance: 20, data: 22, exposure: 0, privilege: 0, environment: 0, combination: 0, business: 0, reach: 0 });
  ...
  it("agent-H-chatbot ×2 → 71 CRITICAL (secondary LLM05 gap scores +5)", () => {
    ...
    expect(r.pillars).toEqual({ toxic: 24, compliance: 25, data: 22, exposure: 0, privilege: 0, environment: 0, combination: 0, business: 0, reach: 0 });
  ...
  it("AGENT_AUTOGEN_DO_NOT_DELETE ×N → 76 CRITICAL (pillar B capped at 30)", () => {
    ...
    expect(r.pillars).toEqual({ toxic: 24, compliance: 30, data: 22, exposure: 0, privilege: 0, environment: 0, combination: 0, business: 0, reach: 0 });
  ...
  it("dev-agent-D / dev-agent-D-test → 67 HIGH (secondary LLM04 gap +5)", () => {
    ...
    expect(r.pillars).toEqual({ toxic: 20, compliance: 25, data: 22, exposure: 0, privilege: 0, environment: 0, combination: 0, business: 0, reach: 0 });
  ...
  it("AWSReservedSSO_FinanceAdmin ×8 (aggregated) → 65 HIGH (data access ×1.1 = 11)", () => {
    ...
    expect(r.pillars).toEqual({ toxic: 24, compliance: 30, data: 11, exposure: 0, privilege: 0, environment: 0, combination: 0, business: 0, reach: 0 });
  ...
  it("agent-J / agent-K → 29 LOW", () => {
    ...
    expect(r.pillars).toEqual({ toxic: 8, compliance: 10, data: 11, exposure: 0, privilege: 0, environment: 0, combination: 0, business: 0, reach: 0 });
