// kmCurve (domain/issueSurvival.ts) went from a scan per distinct event time to one sort and one sweep,
// after production measured 27 s for the two curves behind the Executive week badge. The rewrite
// is only allowed to be FASTER: every published median, quantile and RMST is read off this
// curve, so it must answer bit-for-bit what the old implementation did. The old one is kept
// here verbatim as the oracle.
import { describe, expect, it } from "vitest";
import { kmCurve } from "../src/domain/issueSurvival";
