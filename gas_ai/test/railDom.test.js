...
// app.js as text, the same bargain test/shared.test.js makes for its own thin DOM layers.
// Comment-stripped first, string-aware, so a `//` inside a quoted string survives — mirrors
// `stripCommentsLikeMiddlebox` in esbuild.config.mjs.
//
// Ported from gas/test/railDom.test.js.
...
describe("the rail status dot is a real button, not a hidden span", () => {
  ...
  it("wires the dot's click to navigate to Data, the page the dot's own tip names", () => {
    expect(APP_CODE).toMatch(/rail-status-dot[\s\S]{0,400}navigate\("data"\)/);
  });

  it("gives the dot an aria-label rather than leaving it a bare colour mark", () => {
    expect(APP_CODE).toMatch(/rail-status-dot[\s\S]{0,200}"aria-label":\s*status\.label/);
  });
});
