...
//
// No `Utilities` stub, unlike the sibling gas tool's copy of this file: paramsHash() here goes
// through domain/sha1.ts, a pure implementation, rather than Utilities.computeDigest.
...
describe("the screen says who is about to open the register", () => {
  ...
  it("names THIS product, not the sibling tool's", async () => {
    const { welcomeHtml } = await load();
    const html = welcomeHtml("a@x.com", "https://e/exec?enter=1");
    expect(html).toContain("Wiz SIDEKICK AI");
    expect(html).not.toContain("Wiz Sidekick OS");
  });
});
