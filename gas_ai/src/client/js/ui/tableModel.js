...
import { pageOf } from "../assetQuery.js";
...
/**
 * Cut one page out of a sorted list: `{rows, page, pageCount}`, with `page` clamped into range.
 *
 * RE-EXPORTED, NOT REWRITTEN. This arithmetic already has exactly one definition on the client
 * — assetQuery.js:215 — and that copy is a hand-kept mirror of src/domain/assetTable.ts pinned
 * row-for-row by test/assetQueryMirror.test.ts. Writing the four lines out again here would
 * make a third copy of a thing that already has two by design, and the new one would be the
 * only one nothing checks. The odd-looking import direction (a shared UI module reaching into
 * assetQuery.js) is the price of that, and it is the cheaper mistake: assetQuery.js is
 * DOM-free and imports nothing, so it costs the bundle nothing to reach.
 *
 * The three registers that inline `Math.max(1, Math.ceil(rows.length / PAGE_SIZE))` today —
 * combos.js:663, problems.js:280, config.js:427 — get the clamping for free by adopting it,
 * which is the part they each wrote differently.
 */
export { pageOf };
