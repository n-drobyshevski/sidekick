...
It rebuilds Wiz's security-graph view for the AI landscape: agents, models, guardrails,
MCP servers and their supporting identities, data and compute — as a **depth-limited
node graph** with **toxic combinations highlighted** — plus an inventory that counts
what is open on each asset, a toxic-combinations drill-down, and a concise
"what we scan with Wiz" coverage page.

The app derives three models of its own — the **AI Asset Risk Score (AARS)**, a posture
tier and a problem-tree outcome — and all three are **experimental and confined to the
Scoring Models page**. Measured over a real landscape they do not separate what they claim
to: the same rule and thresholds put 100% of the demo estate in CRITICAL and 97.58% of a
live estate in INFO (`ai/AARS_SCORING_ASSESSMENT.md` §3). Every other page ranks, filters
and reads by **counts** instead — open issues, cloud configuration findings, and compliance
...
| Page | What it shows |
|---|---|
| **Security Graph** | The node graph, asked for with a **query**: `FIND <entity> THAT <relationship> <entity>`, each step a row in a builder, with a `WHERE` segment of editable property filters (24 fields; `is / is not` × `any / all`) on any node in the query — **tags** as key/value pairs, and **domain** as a picker beside them, since a tenant has tens of business domains where it has thousands of distinct tag strings. The builder is an **overlay** behind an *Edit query* toggle — it floats over the canvas rather than sitting above it, so putting the question away gives the answer the whole viewport, and the canvas never reflows either way; the result count stays on the VIEW row, which does not leave. Escape, the card's ✕ or a click on the canvas puts it away, and the choice is remembered for the session (`sessionStorage`, deliberately not a hash param, so a shared link and a saved view replay the question and not the furniture). A **100-node budget** (Settings, 30–400) is a hard ceiling on the payload, counting the "+N more" stubs it also draws, and a capped view offers **Load more** to widen that one view a step at a time (a `maxNodes` hash param, so a widened view is shareable and clears from its chip); per-kind caps collapse high-fanout neighbors into those pills, which expand on demand; a whole-landscape view admits its seeds worst-first in waves so the budget buys paths rather than a field of disconnected dots; a count indicator flags capped views. Every risk signal is a **node on the attack path**, not a flag on a card: `ISSUE`, `INTERNET_EXPOSURE`, `EXCESSIVE_PRIVILEGE`, and `MISSING_GUARDRAIL` (a negated `PROTECTED_BY` edge, drawn dashed) hang off the asset they describe. **Data exposure is drawn as the path Wiz itself draws** — `AI_AGENT → RUNS_AS → SERVICE_ACCOUNT → ALLOWS_ACCESS_TO → BUCKET/DATABASE → HAS_DATA_FINDING → DATA_FINDING` — with the findings collapsed into one aggregate per datastore carrying a count badge and the worst severity, because a bucket with two hundred findings is one fact about that bucket and would otherwise spend the whole node budget. `SENSITIVE_DATA` survives as the **fallback**: where Wiz flags an asset but no path is traversable (the tenant rejected the step, or the grant is expressed some way the query does not walk), the old stub is drawn instead, so one asset never tells the same story twice. Its mark is the data-finding gem, left unfinished. Because they are evidence rather than inventory, a property filter passes them through with their asset, and the grouped layout files them in their asset's block — unless you name a risk kind in the query, which curates them directly. Toxic-combination members get a crimson halo + `TC` badge. Under the default **Smart order** the bands are **clustered**: each attack path (a connected component, split by nearest AI agent where one component holds several) claims a contiguous stripe of slots in every band and the next stripe starts a wider gutter away, so proximity alone groups a path — on the sample tenant that cuts the mean edge length by ~60%. Clusters that share a node (one database read by three agents) are laid side by side, and the run of them **wraps onto shelves** — repeats of the whole band set — chosen to leave the canvas the shape of the viewport, which roughly doubles the zoom "Fit" lands on. Picking an explicit order instead turns clustering off. Keyboard: arrows walk edges/lanes, Enter opens details; a "View as table" fallback carries the same data. |
| **AI Inventory** | Every synced asset with its open-issue count, failing cloud findings, worst issue severity, combo membership and guardrail status. The header reads in three levels rather than as a row of equal tiles: the **census** (AI assets, agents, agentic identities), the **count triad** (open issues, cloud findings, compliance posture fails — the third landscape-wide, because Wiz reports posture per framework/subcategory/policy and never per resource, so it has no per-asset grain and appears in no column), and a **worst-open-issue distribution** as a segmented strip over a stat strip. The strip is also the page's cross-filter — its keys are toggle buttons, so clicking a severity filters the register and the others recede, with a keyboard path a canvas could not offer. It counts **assets by worst issue**, not issues, because the facet it toggles filters rows by exactly that field; a strip counting issues would offer segments a click could not reproduce. Each headline count carries a **change chip** only where history supports one: two points, both carrying a figure for that series, and the last agreeing with the live count. A **filter drawer** (`panel=filters`, live-apply) facets on issue severity, kind (grouped by category, searchable), cloud, region, project, **domain** (the resource's own `Wiz/Domain` tag, beside a count of how many assets carry it at all — an untagged asset is not an option anybody would tick, and an empty facet must not read as "nobody owns any of this") and risk signals; every option carries **how many assets it would still leave**, counted against the *other* dimensions so the number stays useful as you narrow, with a proportion bar beside it. Values inside a dimension OR together; dimensions AND — except **risk signals, which AND inside themselves**, because "agentic *and* missing a guardrail" is the triage question and "either" is not. Zero-yield options are `aria-disabled` (not `disabled`) so they stay focusable and their `0` stays readable, and a selected option is never disabled. **Reaches classified data** joins that group: it is true for an AI asset whose execution identity can read a datastore carrying DSPM findings, and for the datastore itself. It cannot be true for the *identity* — service accounts are unscored, so nothing persists their reach — which is why pairing it with "agentic identity" narrows to nothing rather than to the agentic identities that can read classified data. Applied filters show as dismissible chips with a count badge on the trigger. The register renders as a **sortable table** (every column but Guardrail/Projects, direction in the URL) with each count beside a severity-split micro-bar whose *length* is the volume, or as a **card grid** (`view=cards`). Twelve columns is more than one question needs, so the table carries a **column chooser** — a cog at the right end of the heading row, pinned in view when the table is wider than the window, which is when it matters. It opens the column list; Name is listed but not offered, since a register of counts attached to nothing is unreadable rather than narrower, and the Graph column has no heading to print beside a checkbox. **Cloud, Region and Projects start off** — all three are filter facets, so the reader who cares about them is narrowing rather than scanning, and a project list is the widest and most repetitive cell the table can draw. **Classified data** joins them as a column this register never had: how many classified findings an asset can reach has been on the row all along (the header counts it and the drawer facets on it) with nothing saying which assets or how many each; an unscored identity reads as no answer rather than as zero. The choice rides in the URL as `cols` and is carried by a saved view, and what it holds is the columns **removed**, never the ones kept — a link listing what to keep would hide any column added after it was saved, silently, and nobody reports a column they have never seen. Hiding one changes no row, no count and no facet: it is a reading preference, so the table repaints and nothing else on the page moves. The cog marks itself when the table is narrowed and its accessible name says by how many, because a missing column with no sign of why reads as a bug. **Saved views** keep filter sets in `localStorage` — the URL already encodes the whole state, so sharing is a copied link. The trend under the History heading charts the three counts over time, one point per sync and not backfillable — and **nullable per series**, because `issue_count` has been recorded since the first sync this app ever ran while the finding and posture-fail columns start at the sync after they were appended. A series with no figure for a sync draws a **gap, not a zero**: plotting zero would show a landscape with no failing controls until the day we started counting them. The table is **paged** (25–250 rows, page and size in the URL); a small landscape ships in one payload and filters, sorts and counts facets in the browser, a large one does all three server-side — the header, the filter vocabulary and the facet counts describe the whole inventory either way. |
| **Priorities** | Every unresolved issue and every open configuration finding, ranked together on one scale — Wiz's severity, then how soon it is due, then how long it has been open. Opens on **By action**: the union collapsed by remediation action through a greedy marginal set-cover, under the headline it exists for — "N open problems collapse to M actions — the top 10 close K%" — which is pure counting over the true cover order. **By problem** is the one-row-per-problem register. Both carry the owning **domain**, and the action rollup carries the SET of domains one action spans — the headline says N problems collapse to M actions, and that column says whose problems they are, which is the coordination cost a count alone hides. Neither carries a verdict column: the ACT/ATTEND/TRACK\* cascade that used to lead both is experimental and lives on Scoring Models. |
...
warm window, a grace period long enough to cross the gap), on `ArrowRight` from the keyboard,
and on the first tap where there is no hover at all. Its `→|` control **pins**
...
exactly like "nothing here". Picking one stores the scope server-side and re-boots, so
...
2. **Normalize + enrich** (once per sync, persisted): responses become typed
   ...
   **The score is still computed and still persisted — it is simply published nowhere but
   the Scoring Models page.** That is the whole shape of the isolation: a model under
   calibration has to be visible to the page that calibrates it, and invisible everywhere
   a reader would take it for a fact about an asset. The same holds for the posture tier
   and the problem outcome folded on beside it. The two counts every other page reads
   (`openIssues` / `openFindings`) are NOT persisted — they are a read-time fold
   (`graphEnrich.withOpenCounts`), because both populations live in their own tabs and move
   without `ai_assets` being rewritten.
   ...
   Scoring Models page; under it pillar C takes four values on the sample landscape rather than two.
...
4. **Risk topology and the business domain** are derived on READ, never persisted: the `with*Nodes` helpers in
   ...
   assets with no traversable path to a classified store. `withDomains` joins them,
   resolving each resource's owning business domain from its `Wiz/Domain` tag — all of
   it through `syncStore.withReadDerivations`, the one helper that exists so a read path
   cannot pick up half of them. Read-time means already-synced graphs gain them without a
   re-sync, and they never leak into the `ai_assets` tab the inventory reads.
...
2. For live syncs, set Script Properties (Project Settings → Script Properties):
   ...
   Optional: `WIZ_DOMAIN_TAG_KEY` names the resource tag whose value is the owning business
   domain. It defaults to `Wiz/Domain` and the key is matched case-insensitively, so this only
   needs setting by a tenant that spells the key differently rather than merely
   differently-cased. It is read on every request, so changing it repaints without a re-sync.
...
- **Tags arrive in two shapes, and the bag is the only one that carries `Wiz/Domain`.**
  `cloudResourcesV2` selects `tags { key value }` and gets an ARRAY; a `graphSearch` entity's
  `properties` bag holds the same fact as an OBJECT MAP, because a bag has no sub-selection.
  Both appear on the same node in `exemples/get_ai_agents_reponse.js` (array at :4517, map at
  :4462) — and the real inventory capture sends flat `"tags": null` on 39 of its 40 nodes while
  the bag carries the pair. `graphTypes.entityTags` reads both and unions them, the bag winning
  a collision. It deliberately does NOT go through `entityField`, which returns the flat value
  the moment it is not `undefined` and would therefore stop at that null. **Do not add `tags` to
  `ENTITY_FIELDS`** — `test/wizQueriesAi.test.ts` forbids it, the bag already answers, and every
  addition to that shared constant is a way for all seven traversals to be rejected at once.
  `tagPairs` answers `undefined` and never `[]`, because `mergeParts` copies any truthy value
  and an empty array would erase what an earlier step established.
- **A domain is derived on read, never a column.** `WIZ_DOMAIN_TAG_KEY` (default `Wiz/Domain`,
  matched case-insensitively) is a Script Property, so `settingsStore.saveSettings` never sees
  it and `DATA_VERSION` never bumps for it. That is why the resolved key joins the build stamp
  in `serverCache`'s key and `derivedAssetsMemo` keys on it beside `bandKey` — otherwise an
  operator who fixed the key would keep seeing the old answer for six hours with no sync to run.
  Baking the value into `ai_assets` would mean a re-sync to change a string, which is the trap
  `gas/`'s `_domain` was built to avoid, and the raw material (`tags_json`) is already in the
  sheet. Absence is published as `domainCoverage`, a count, rather than as a per-row sentinel:
  `AI_ASSET_PROPERTIES` is optional and swallows an HTTP 400, so "no domains" has to be
  distinguishable from "we never successfully asked".
...
- **A page cap is recorded, not silent.** `MAX_PAGES` used to stop a step with a bare
  `break`, so a step that ran out of pages was indistinguishable from a landscape that has
...
- **A rule change reaches the data two different ways.** An AARS **level** is re-derived
  ...
  which the Scoring Models page flags in its own toolbar ("Scores stale" / "Scores mixed").
  "Recompute scores" re-runs enrichment over data already in the sheet — issue framework
  codes, finding framework codes, per-asset inputs — and makes **zero Wiz API calls**. It
  writes no `sync_history` row: a rescore is not a sync, and the trend must not gain a point
  for a landscape that never moved. The inventory's trend charts COUNTS now, which is why it
  needs no rule-version caveat at all: two counts of open issues are on the same scale
  however the models are tuned.
...
- **The Priorities rank cube (P11) collapses `ruleWeightKey` to a weight NUMBER, not a rule
  id, and that is only valid while this settings panel edits no per-rule weight.** Today the
  draft can only move the four blend shares, the two weight tables, `epssThreshold` and
  `timeSource` — never `ruleWeights` or `defaultRuleWeight` — so two rows landing on the same
  weight number score identically under every rule the page can produce, and the cube
  (`src/domain/settingsImpact.ts`'s `RankCubeTuple`, built by `rankCubeModel.js`) keys on that
  number instead of paying for one cell per rule id, of which a live tenant carries hundreds.
  The module's own P11 header states the reversal: the moment a later package adds a
  per-rule-weight editor to this panel, the tuple's first component has to become the rule id
  itself, and the cube grows from a handful of cells to a cell per rule id — whoever builds
  that editor should re-read the header before assuming the cube still fits inside
  `CacheService`'s 100 KB ceiling unchanged.
