...
2. **Normalize + enrich** (once per sync, persisted): responses become typed
   ...
   CRITICAL down to INFO — see `ai/custom_score.md`, whose applied table pins the unit
   tests). Those numbers are `DEFAULT_AARS_RULE`; the rule actually in force comes from
   the `settings` tab, and the asset's scoring **inputs** are persisted beside its score
   (`aars_input_json`) so a later rule change re-prices exactly these gaps.
