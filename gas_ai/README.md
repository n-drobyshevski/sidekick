...
2. **Normalize + enrich** (once per sync, persisted): responses become typed
   ...
   assets). `AARS_V2_RULE` is a calibrated preset combining them, loadable from the
...
3. **Persist**: wholesale rewrite of the `ai_assets` / `ai_edges` / `ai_issues` /
   `ai_data_findings` tabs, a gzipped graph snapshot to Drive (the fast read path), then
   ...
   holds failing compliance *controls*, prices AARS pillar B and counts as
   `complianceGaps`, and a classification finding folded in would inflate both.
