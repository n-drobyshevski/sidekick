...
| **AI Inventory** | Every synced asset with its AARS score and severity, issue severity, combo membership, guardrail status; KPI cards (guardrail coverage %, critical/high counts). Two charts sit inline above it: the AARS-severity distribution now, and that distribution over time (one point per sync, recorded going forward — it cannot be backfilled). Both omit INFO, which is the biggest bucket in a healthy estate and flattens the levels worth watching; each says so under its title. The table is **paged** (25–250 rows, page and size in the URL); a small estate still ships in one payload and filters in the browser, a large one is filtered and paged server-side — the KPIs, the AARS-severity chart and the filter options always describe the whole inventory either way. |
...
| **AARS Rules** | The scoring model itself, inspectable and tunable: pillar-A severity points, the multi-issue multiplier and cap; pillar-B's **ordered gap-pricing cascade** (first match wins, with the fallback that prices tenant-specific finding shortIds) and cap; pillar-C exposure points and the 5Rs amplifier; and the level thresholds. The defaults are `ai/custom_score.md` exactly. Every edit is previewed against the live inventory before it is saved — proposed-vs-current level counts plus the assets that move — and a sandbox scores a hypothetical asset through the server, so the model has one implementation, not two. Export/import the rule as JSON. |
