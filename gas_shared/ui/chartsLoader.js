...
// costs when it fails: 1 and 2 fail synchronously. `ui/dom.js` already creates `blob:` URLs
// for the CSV export, which is evidence that creation works — not that a blob may be a script
// source, which is a different directive.
//
...
import { chartTipHandler } from "./tip.js";
 ...
 *
 * A REJECTION IS CACHED HERE, unlike the per-area memo in scans.js, and the difference is what
 * the failure means. A failed fetch there is a transport blip and re-opening should retry;
 * a refusal here is a property of the deployment's policy, and re-running three blocked
 * mechanisms on every navigation between the two chart routes would cost an RPC and a console
 * error each time to reach the same answer.
 */
let pending = null;
...
async function start() {
  ...
  const api = await run(src);
  // The app's one hover vocabulary, handed over rather than imported — see charts.js.
  api.setChartTipHandler(chartTipHandler);
  return api;
}
...
function byScriptText(src) {
  ...
  // Removed either way: it has already run or it never will, and a 171 KB text node in the
 ...
 *
 * BOTH CALL SITES ALREADY HAVE A NO-CHART BRANCH — inventory.js draws `.chart-empty` when a
 * tenant has fewer than two syncs, problems.js a `.page-hero-sub` when fewer than three
 * actions close the board — so this reuses that vocabulary rather than inventing a third. It
 * replaces the `.chart-box` and nothing else: the card keeps its heading, its counts and its
 * scope notes, which are the parts that were never the chart's to carry.
 *
