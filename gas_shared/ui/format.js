 ...
 *
 * Note the sign. gas_ai's assetQuery.js has its own `sevRank` on an INVERTED scale (higher
 * = worse) because it is a hand-kept mirror of that app's src/domain/assetTable.ts. It looks
 * like this one and means the opposite; do not fold them together.
 */
 ...
 *
 * ONE COPY, because there were two and they disagreed. In gas_ai, `problemView.js` had the
 * sentence above and the right sentinel; `comboView.js` had the same four lines returning
 * `Infinity`, so that register's Due column declined to order its undated rows until the fix
 * landed. A divergence is the argument for sharing, not the cost of it — and both live here
 * rather than in tableModel.js because this is the file that already owns the other "turn a
 * column into a number" helper.
 *
 * Both arrived from gas_ai/src/client/js/ui/format.js when this package was cut: they are
 * pure, they name nothing app-specific, and they were the two helpers the fork had left
 * behind on one side of the split.
 *
