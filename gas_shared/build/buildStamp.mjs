...
// happened in a sibling package, which is why this file exists.
//
...
//
// The obvious alternative is a git SHA baked in at build time. There used to be one —
// __BUILD_COMMIT__ and __BUILD_DATE__ — on the reasoning that a content hash cannot be
// looked up but a SHA can. Both were removed: a commit stamp inside a committed artifact
// is a fixpoint that does not exist. dist/ is tracked in this repo, so the order is always
// build, then commit src and dist together. At build time the commit that will contain
// the build has no SHA yet, so the bundle can only ever name its own parent — and
// rebuilding to correct that produces a new commit, which makes it stale again. The
// symptom was that every `npm run check` after any commit left dist/ dirty by exactly one
// line, forever, with no sequence of commits that could settle it.
//
