...
export async function renderAccessPanel() {
  // Never let this panel take the Settings page down with it. It is one section among eight,
  // and it is the only one whose absence is already a normal outcome — so a failure here
  // renders nothing, exactly as a non-editor does, rather than replacing retention, severity
  // scope and the domains editor with "This page failed to load". (It did exactly that once,
  // over a mistyped RPC name; test/entryPoints.test.ts now pins those names.)
  let info;
  try {
  ...
  let savedUsers = users.join(",");
  let savedAdmins = admins.join(",");
  ...
  function addRow(placeholder, add) {
    ...
    return el("div", { class: "sub-add" }, input,
  ...
  async function save() {
    ...
    try {
      ...
      savedAdmins = admins.join(",");
      if (scopedBlock.sync) scopedBlock.sync(fresh);
      drawUsers();
      drawAdmins();
      refreshDirty();
    } catch (e) {
  ...
  // A scoped save can move people OUT of the people list (the tiers are exclusive), so the
  ...
  const accessPanel = settingsPanel({
    title: "Access",
    description:
    ...
    body: [
      el("div", { class: "scope-block" },
        el("span", { class: "label" }, "People"),
        usersHost),
      el("div", { class: "scope-block scope-block--divided" },
        el("span", { class: "label" }, "Admins"),
