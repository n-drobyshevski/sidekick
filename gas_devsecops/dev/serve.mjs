...
// harness that reads it, and the package root is where a .env lives in every other project.
...
async function proxyFetch(req, res) {
  ...
  const c = credentials();
  if (!c.mode) {
    reply({ error: "no credentials configured — fill in gas_devsecops/dev/.env.local" });
    return;
  }
  let spec;
  try {
...
createServer(async (req, res) => {
  ...
  console.log(`Wiz Sidekick DevSecOps local dev: http://localhost:${PORT}`);
  console.log("Edit gas_devsecops/src/** and refresh — each page load rebuilds and reseeds.");
  const c = credentials();
  if (c.mode) {
    console.log(
    ...
    console.log(
      ...
      "gas_devsecops/dev/.env.local and refresh for real tenant data.",
    );
  }
});
