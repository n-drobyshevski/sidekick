...
//
// PORTED FROM gas/test/accessAdmin.test.ts, which this project's own access.ts already
// pointed at ("test/accessAdmin.test.ts exists to fail the moment it is blurred") while the
// fork had never brought it across — a dangling reference to a guarantee that did not exist
// here. The endpoints it covers arrived with the Access panel; the rules it pins were already
// implemented in access.ts and simply untested.
//
...
// The ledger side of api.ts is irrelevant here and drags in Sheets/Drive on import. Only the
// authorization path is under test, and it touches none of this.
...
vi.mock("../src/server/readModelStore", () => ({
  durablyCached: (_n: string, _p: unknown, compute: () => unknown) => compute(),
  __resetMemosForTest: () => {},
}));
vi.mock("../src/server/locks", () => ({
  LedgerBusyError: class extends Error {},
  withScriptLock: (fn: () => unknown) => fn(),
  recoverIfNeeded: () => {},
}));
