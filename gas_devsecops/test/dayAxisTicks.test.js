// PORTED FROM `gas/test/dayAxisTicks.test.js` ALONGSIDE THE CODE IT GUARDS. This register's
// `charts.js` carried `dayAxis` without the tick thinning, so every day-axis chart here —
// "Open vs resolved" on Scan History, coverage/efficiency on Programme, and now the two KM
// trends — printed its last two date labels on top of each other. The fixtures below are
// `gas/`'s measurements off `gas/`'s harness rather than this app's, and they stay that way:
// the defect is a property of `fmtDay`'s eleven characters and Chart.js's five-character
// budget, which both registers share exactly.
//
...
// A DYNAMIC IMPORT, AND THAT IS THIS PORT'S ONE EDIT. Unlike `gas/`'s copy, this register's
// `charts.js` reads `window.matchMedia` at MODULE SCOPE (its `reducedMotion` const), so a
// static import of it from a suite with no `environment` throws before a single test runs.
// `charts.test.js` solves the same problem by re-importing inside a helper; here the two
// functions under test are pure, so stubbing the one global the module evaluation touches and
// awaiting the import at top level is enough.
...
} = await import("../src/client/js/charts.js");
